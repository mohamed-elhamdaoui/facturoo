...
<div x-data="{ openDeleteModal: false, deleteUrl: '' }">
    ...
    <!-- Alert Success/Error -->
    @if(session('success'))
        <div class="mb-6 p-4 bg-green-50 border border-green-200 text-green-800 rounded-lg text-sm flex items-center">
    ...
    @endif
    @if(session('error'))
        <div class="mb-6 p-4 bg-red-50 border border-red-200 text-red-800 rounded-lg text-sm flex items-center">
            <svg class="w-5 h-5 mr-2 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
            {{ session('error') }}
        </div>
    @endif
    ...
    <div class="bg-white shadow-sm border border-slate-200 rounded-xl overflow-hidden" id="clientsContainer">
                ...
                <table class="min-w-full divide-y divide-slate-200">
                    <thead class="bg-slate-50">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Nombre de Factures</th>
                            ...
                            <tr class="client-row hover:bg-slate-50/80 transition-colors" data-name="{{ strtolower($client->name) }}">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        <button type="button" 
                                                @click.prevent="deleteUrl = '{{ route('clients.destroy', $client) }}'; openDeleteModal = true"
        ...
        <!-- Empty Filter State -->
        <div id="emptyFilterState" style="display: none;" class="text-center py-12">
    ...
    <div x-show="openDeleteModal" 
        ...
        <div class="relative max-w-md w-full bg-white rounded-2xl p-6 shadow-2xl border border-slate-100 transform transition-transform"
            ...
            <div class="flex items-start gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-slate-500 mt-2">Êtes-vous sûr de vouloir supprimer ce client ? Cette action est irréversible et ne pourra pas être effectuée si des factures y sont liées.</p>
                </div>
            </div>
